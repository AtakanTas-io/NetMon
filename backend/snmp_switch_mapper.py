...
def fetch_switch_mac_table(ip: str, community: str) -> Dict[str, str]:
    ...
    try:
        iterator = nextCmd(
            ...
            ContextData(),
            ObjectType(ObjectIdentity('1.3.6.1.2.1.17.4.3.1.2')), # BRIDGE-MIB::dot1dTpFdbPort
...
def update_switch_mac_tables(devices: List[Dict]):
    ...
    switches = [d for d in devices if d.get("type") in ("switch", "router") or 161 in (d.get("classification", {}).get("open_ports") or [])]
    ...
    try:
        ...
        for dev in devices:
            ...
            if mac in _mac_to_switch_port:
                ...
                conn.execute(
                    "UPDATE known_devices SET switch_ip=?, switch_port=? WHERE mac=?",
                    (sw_ip, sw_port, mac)
                )
        conn.commit()
        conn.close()
