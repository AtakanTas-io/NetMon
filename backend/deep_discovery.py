    ...
    if HAS_WINRM and username and password:
        try:
            ...
            if res.status_code == 0 and res.std_out:
                ...
                result["software"] = {"os_name": data.get("OS"), "os_build": data.get("OSVersion"), "installed_programs": []}
...
def scan_linux_deep(
    ...
    if HAS_PARAMIKO and username:
        ...
        try:
            ...
            cpu_name = run_cmd("lscpu 2>/dev/null | grep 'Model name' | cut -d':' -f2 | sed 's/^[[:space:]]*//' || cat /proc/cpuinfo | grep 'model name' | head -n1 | cut -d':' -f2")
            ...
            firewall = run_cmd("if command -v ufw >/dev/null 2>&1; then ufw status 2>/dev/null | head -n1; elif command -v firewall-cmd >/dev/null 2>&1; then firewall-cmd --state 2>/dev/null; elif command -v nft >/dev/null 2>&1; then nft list ruleset 2>/dev/null | head -n1; fi")
            disk_info = run_cmd("df -B1 -P -x tmpfs -x devtmpfs 2>/dev/null | tail -n +2")
            pkgs = run_cmd("dpkg-query -W -f='${Package} ${Version}\n' 2>/dev/null | head -n 500 || rpm -qa 2>/dev/null | head -n 500")
            ...
            result["software"] = {
                "os_name": os_rel or None,
                "installed_programs": installed_sw
            }
