...
USER_DATA_DIR = (
    ...
    if os.environ.get("NETMON_DATA_DIR", "").strip()
    else Path(os.path.expanduser("~")) / ".netmon"
)
        ...
        base.update(
            {
                ...
                "diagnosis": "[SİMÜLASYON] Modem/router'a ulaşılamıyor — internet kesintisi.",
                "status": "fail",
            }
        )
        ...
        base.update(
            {
                ...
                "diagnosis": "[SİMÜLASYON] Bağlantı çalışıyor ama gecikme çok yüksek (340 ms).",
                "status": "warn",
            }
        )
        ...
        base.update(
            {
                ...
                "dns_test": {"success": True, "result": "142.250.187.14"},
                "diagnosis": "[SİMÜLASYON] Kısmi paket kaybı tespit edildi (%28).",
                "status": "warn",
            }
        )
        ...
        base.update(
            {
                ...
                "diagnosis": "[SİMÜLASYON] İnternet bağlantınız var ama alan adları çözümlenemiyor (DNS hatası).",
                "status": "fail",
            }
        )
        ...
        base.update(
            {
                ...
                "dns_test": {"success": True, "result": "142.250.187.14"},
                "diagnosis": "[SİMÜLASYON] Bağlantı sağlıklı, yalnızca yoğun trafik örneği gösteriliyor.",
                "status": "ok",
            }
        )
    else:
...
def init_db():
    ...
    """)
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_inventory_history_asset ON inventory_history(asset_id, created_at DESC)"
    )
...
def _check_traffic_anomaly(now: float, total_bps: float, conn: sqlite3.Connection):
    ...
    if (
        ...
        conn.execute("INSERT OR REPLACE INTO alerts (ts, level, message) VALUES (?, ?, ?)", (now, "warning", message))
        conn.commit()
        manager.broadcast_threadsafe(
            {"type": "alert", "ts": now, "level": "warning", "message": message, "simulated": False}
        )
...
def traffic_sampler_loop(stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        sim = simulation_state["active"] and SCENARIOS.get(simulation_state["scenario"], {}).get("affects") in (
            "traffic",
            "both",
        )
        ...
        if not sim:
            conn.execute(
                "INSERT OR REPLACE INTO traffic (ts, wifi_sent, wifi_recv, eth_sent, eth_recv) VALUES (?, ?, ?, ?, ?)",
        ...
        manager.broadcast_threadsafe(
            {
                "type": "traffic",
                "ts": now,
                "sent": wifi_sent + eth_sent,
                "recv": wifi_recv + eth_recv,
                "wifi_sent": wifi_sent,
                "wifi_recv": wifi_recv,
                "eth_sent": eth_sent,
                "eth_recv": eth_recv,
    ...
    while not stop_event.is_set():
        try:
            ...
            if prev_status is not None and prev_status != cur_status:
                ...
                if not sim:
                    conn.execute(
                        "INSERT OR REPLACE INTO alerts (ts, level, message) VALUES (?, ?, ?)", (now, level, message)
                    )
                    conn.commit()
                manager.broadcast_threadsafe(
                    {"type": "system_alert", "ts": now, "level": level, "message": message, "simulated": sim}
                )
...
try:
    from .dhcp_monitor import (
        start_dhcp_monitor,
        stop_dhcp_monitor,
        configure_authorized_dhcp_provider,
...
CAPABILITY_CATALOG = (
    {
        ...
        "title": "Otomatik Ağ Keşfi",
        "permission": "discovery.schedule.manage",
    ...
    {
        "id": "reports",
        "title": "Operasyon ve SLA Raporları",
    ...
    },
    {
        ...
        "title": "Konfigürasyon Yedekleme (NCM)",
        "permission": "ncm.manage",
        "roles": ["Sistem Yöneticisi", "NOC Operatörü"],
    ...
    {
        "id": "locations",
        "title": "Şube, Bina ve Lokasyon Haritası",
        ...
        capabilities.append(
            {
                **item,
                ...
                "state": "ready"
                if allowed and readiness.get(item["id"], False)
...
def get_system_readiness(user: dict = Depends(get_current_user)):
    ...
    items = [
        ...
        item(
            "live_telemetry",
            "Canlı sistem ve trafik telemetrisi",
            "ready" if HAS_PSUTIL else "unavailable",
            "Fiziksel arayüz, soket, CPU ve bellek sayaçları kullanılabilir."
            if HAS_PSUTIL
            else "psutil bağımlılığı kurulu değil.",
            category="core",
        ),
        item(
            "discovery",
            "Ağ keşfi",
            "ready" if nmap_ready else "degraded",
            "Nmap ve yerleşik keşif motoru kullanılabilir."
            if nmap_ready
            else "Nmap bulunamadı; ARP, ICMP ve yerleşik yöntemlerle sınırlı keşif çalışır.",
            "Nmap kurulumunu doğrulayın." if not nmap_ready else "",
            "discovery",
        ),
        item(
            "speedtest",
            "İnternet hız testi",
            "ready" if speedtest is not None else "unavailable",
            "speedtest-cli kullanılabilir." if speedtest is not None else "speedtest-cli bağımlılığı kurulu değil.",
            category="diagnostics",
        ),
        item(
            "windows_inventory",
            "Windows derin envanteri",
            "ready"
            if (deep_discovery.HAS_WMI or deep_discovery.HAS_WINRM)
            and settings.get("wmi_username")
            and settings.get("wmi_password")
            else "needs_configuration"
            if (deep_discovery.HAS_WMI or deep_discovery.HAS_WINRM)
            else "unavailable",
            "WMI/WinRM bağımlılığı ve servis hesabı hazır."
            if (deep_discovery.HAS_WMI or deep_discovery.HAS_WINRM)
            and settings.get("wmi_username")
            and settings.get("wmi_password")
            else "Uzak Windows envanteri için WMI/WinRM bağımlılığı ve salt-okuma servis hesabı gerekir.",
            "Ayarlar > Yetkili Envanter",
            "inventory",
        ),
        item(
            "ssh_inventory",
            "SSH / NCM",
            "ready"
            if deep_discovery.HAS_PARAMIKO and settings.get("ssh_username") and settings.get("ssh_password")
            else "needs_configuration"
            if deep_discovery.HAS_PARAMIKO
            else "unavailable",
            "Paramiko ve SSH servis hesabı hazır."
            if deep_discovery.HAS_PARAMIKO and settings.get("ssh_username") and settings.get("ssh_password")
            else "SSH bağımlılığı ile salt-okuma kullanıcı/parola gerekir.",
            "Ayarlar > Yetkili Envanter",
            "inventory",
        ),
        item(
            "snmp_inventory",
            "SNMP ağ cihazı envanteri",
            "ready"
            if snmp_dependency and settings.get("snmp_community")
            else "needs_configuration"
            if snmp_dependency
            else "unavailable",
            "SNMP bağımlılığı ve community hazır."
            if snmp_dependency and settings.get("snmp_community")
            else "pysnmp bağımlılığı ve salt-okuma community gerekir.",
            "Ayarlar > Yetkili Envanter",
            "inventory",
        ),
        item(
            "active_directory",
            "Active Directory oturum açma",
            "ready" if settings.get("ad_server") and settings.get("ad_domain") else "needs_configuration",
            "AD sunucusu ve domain yapılandırıldı."
            if settings.get("ad_server") and settings.get("ad_domain")
            else "AD sunucusu ve domain alanları henüz yapılandırılmadı.",
            "Ayarlar > Active Directory",
            "access",
        ),
        item(
            "dhcp_monitor",
            "Rogue DHCP izleyicisi",
            "ready" if dhcp_state.get("running") else "error" if dhcp_state.get("error") else "needs_configuration",
            "UDP/68 dinleyicisi çalışıyor."
            if dhcp_state.get("running")
            else (dhcp_state.get("error") or "DHCP izleyicisi henüz başlamadı."),
            "UDP/68 kullanımını ve yönetici yetkisini kontrol edin.",
            "security",
        ),
        item(
            "firewall",
            ...
            "ready"
            if firewall_state.get("state") == "enabled"
            else "error"
            if firewall_state.get("state") == "disabled"
            else "degraded",
            "İşletim sistemi profillerinde açık."
            if firewall_state.get("state") == "enabled"
            else "İşletim sistemi profillerinde kapalı."
            if firewall_state.get("state") == "disabled"
            else "Durum işletim sisteminden doğrulanamadı.",
            "Windows güvenlik duvarı profillerini kontrol edin.",
            "security",
        ),
        item(
            "web_filter",
            "Web filtresi / proxy log entegrasyonu",
            "unavailable",
            "Bu sürümde gerçek web filtresi veya proxy log bağlayıcısı yok.",
            category="security",
        ),
        item(
            "siem",
            "SIEM ve otomatik engelleme",
            "unavailable",
            "Bu sürüm SIEM alarmı açmaz ve firewall kuralı uygulamaz.",
            category="security",
        ),
    ]
    counts = {
        state: sum(1 for entry in items if entry["state"] == state)
        for state in ("ready", "degraded", "needs_configuration", "unavailable", "error")
    }
    return {
        ...
        if counts["needs_configuration"] or counts["unavailable"] or counts["degraded"]
        else "ready",
...
def _discover_configured_devices() -> list[dict]:
    ...
    for subnet in subnets:
        ...
        for device in discovered_devices:
            ...
            if not ip:
                continue
            if ip not in merged or len(device.get("discovery_sources", [])) > len(
                merged[ip].get("discovery_sources", [])
            ):
                merged[ip] = device
...
def merge_scan_into_inventory(scanned: list[dict]) -> list[dict]:
    ...
    unique_devices = [
        ...
        if d.get("mac")
        or d.get("hostname")
        or d.get("status") in ("online", "discovered")
        or d.get("is_self")
        or d.get("is_gateway")
    ]
...
def get_overview(user: dict = Depends(get_current_user)):
    ...
    latencies = [
        float(d.get("latency")) for d in devices_list if d.get("latency") is not None and float(d.get("latency")) > 0
    ]
    avg_device_latency = round(sum(latencies) / len(latencies), 1) if latencies else None
    packet_losses = [
        float(d.get("packet_loss"))
    ...
    return {
        ...
        "latency": {
            "average": avg_device_latency
            if avg_device_latency is not None
            else _last_status.get("internet_test", {}).get("average")
        },
...
def get_topology(user: dict = Depends(get_current_user)):
    ...
    lan_status = (
        "online" if gateway_status == "online" else ("discovered" if gateway_status == "discovered" else "unknown")
    )
    nodes = [
        {"id": "internet", "label": "INTERNET", "type": "internet", "status": internet_status},
        {
            ...
            "ip": gateway,
            "hostname": (gateway_dev or {}).get("hostname"),
            "is_gateway": True,
            "connectivity_status": (gateway_dev or {}).get("connectivity_status", gateway_status),
        },
        {
            "id": "lan",
            "label": "LAN / ERİŞİM KATMANI",
            "type": "switch",
            "status": lan_status,
            "logical": True,
            "note": "Fiziksel switch/port keşfedilmedi; bu düğüm mantıksal LAN segmentini temsil eder.",
        },
    ]
    edges = [
        {
            "from": "internet",
            "to": "gateway",
            "status": internet_status if gateway_status == "online" else gateway_status,
            "kind": "uplink",
            "layer": "l3",
            "logical": True,
            "source_port": None,
        ...
        if device_type == "switch":
            ...
            for p_num in range(1, 25):
                ...
                connected_dev = next(
                    (
                        d
                        for d in unique_devices_list
                        if (d.get("switch_ip") == sw_ip or not d.get("switch_ip"))
                        and (d.get("switch_port") == p_name or d.get("switch_port") == f"Port {p_num}")
                    ),
                    None,
                )
                ports_matrix.append(
                    {
                        ...
                        "connected_name": connected_dev.get("hostname")
                        or connected_dev.get("friendly_name")
                        or connected_dev.get("vendor")
                        if connected_dev
                        else None,
                ...
                ports_matrix.append(
                    {
                        ...
                        "connected_name": (gateway_dev or {}).get("hostname") or "Gateway Firewall"
                        if is_uplink
                        else None,
    ...
    for idx, dev in enumerate(unique_devices_list):
        ...
        edge_status = (
            ...
            else ("discovered" if dev.get("status") == "discovered" else dev.get("status", "unknown"))
        )
    ...
    return {
        ...
        "meta": {
            ...
            "note": "Gercek switch/port topolojisi kullaniliyor."
            if physical_switch_discovered
            else "Fiziksel switch kesfedilmedi; LAN mantiksal gosterimdir.",
        },
    }
...
def enrich_devices(devices: list[dict]) -> list[dict]:
    ...
    for device in devices:
        if (
            (device.get("is_self") or device.get("ip") == local_ctx.get("local_ip"))
        ...
        if mac:
            ...
            if row:
                ...
                if (
                    hostname
                    and local_hostname
                    and hostname.casefold() == local_hostname.casefold()
                    and mac != local_mac
                ):
                    hostname = None
                ...
                conn.execute(
                    ...
                    (
                        ...
                        device.get("latency"),
                        device.get("packet_loss"),
                        device.get("last_arp_seen"),
                        device.get("last_icmp_seen"),
                        device.get("last_hostname_seen"),
                        vendor or None,
                        json.dumps(device.get("discovery_sources", []), ensure_ascii=False),
                        device.get("connectivity_status", "unknown"),
                        device.get("identification_status", "unknown"),
                        current_network,
                        json.dumps(classification.get("open_ports", [])),
                    ),
                )
        ...
        device.update(
            {
                ...
                "connectivity_status": device.get(
                    "connectivity_status", "online" if device.get("status") == "online" else "unknown"
                ),
                "identification_status": device.get(
                    "identification_status", "identified" if device_type != "unknown" else "unknown"
                ),
                "identification_reason": device.get("identification_reason")
                or (
                    "Cihaz tipi birden fazla kanıta göre belirlendi."
                    if device_type != "unknown"
                    else "Cihaz tipi için yeterli kanıt bulunamadı."
                ),
            }
        )
        ...
        result.append(
            {
                ...
                "identification_status": saved_identification
                or ("identified" if (saved_type and saved_type != "unknown") else "unknown"),
    ...
    try:
        conn.execute(
            "DELETE FROM known_devices WHERE (mac IS NULL OR mac = '') AND last_ip IS NOT NULL AND last_ip IN (SELECT last_ip FROM known_devices WHERE mac IS NOT NULL AND mac != '')"
        )
    except Exception:
        pass
...
def get_nmap_status(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "message": (
            ...
            if available
            else "Nmap bulunamadı. Yerleşik tarama motoru kullanılıyor."
        ),
    }
...
def rename_device(body: DeviceRenameRequest, user: dict = Depends(require_permission("devices.manage"))):
    ...
    manager.broadcast_threadsafe(
        {"type": "devices", "devices": _devices_cache.get("data", []), "ts": _devices_cache.get("ts", 0)}
    )
    return {"ok": True}
        ...
        "devices": [
            {
                ...
                "hostname": r[2],
                "type": r[3],
                "classification_source": r[4] or "auto",
                "owner": r[5] or "",
                "notes": r[6] or "",
                "first_seen": r[7],
                "last_seen": r[8],
...
def _infer_verified_device_type(dev: dict, inventory: dict, source: str | None = None) -> tuple[str | None, float]:
    ...
    if any(token in source_text for token in ("wmi", "winrm", "windows", "cim")):
        ...
        if (
            product_type in {2, 3}
            or (domain_role is not None and domain_role >= 2)
            or "windows server" in identity_text
        ):
            return "server", 0.99
    ...
    if "ssh" in source_text or any(
        token in identity_text for token in ("linux", "ubuntu", "debian", "rhel", "centos", "rocky")
    ):
        ...
        if (
            current_type == "server"
            or ports.intersection(server_ports)
            or any(token in name for token in ("server", "srv", "nas", "storage", "dc-"))
        ):
            return "server", 0.95
...
def _apply_verified_inventory_identity(dev: dict, inventory: dict, source: str | None = None) -> str | None:
    ...
    system = inventory.get("system") if isinstance(inventory.get("system"), dict) else {}
    verified_hostname = inventory.get("computer_name") or system.get("computer_name") or system.get("sys_name")
    ...
    evidence = [
        item for item in evidence if not (isinstance(item, dict) and item.get("source") == "verified_inventory")
    ]
...
def _sync_normalized_inventory(dev: dict, inventory: dict, source: str | None = None):
    ...
    if not row and mac:
        row = conn.execute(
            "SELECT asset_id, first_seen FROM inventory_assets WHERE lower(mac_address)=lower(?)", (mac,)
        ).fetchone()
    if not row and hostname:
        row = conn.execute(
            "SELECT asset_id, first_seen FROM inventory_assets WHERE lower(hostname)=lower(?) ORDER BY last_seen DESC LIMIT 1",
            (hostname,),
        ).fetchone()
    if not row and ip:
        row = conn.execute(
            "SELECT asset_id, first_seen FROM inventory_assets WHERE ip_address=? ORDER BY last_seen DESC LIMIT 1",
            (ip,),
        ).fetchone()
    ...
    if row:
        ...
        old = conn.execute(
            "SELECT hostname,ip_address,mac_address,vendor,device_type,os_name,os_version,status,identity_key FROM inventory_assets WHERE asset_id=?",
            (asset_id,),
        ).fetchone()
        oldmap = dict(
            zip(
        ...
        update_identity = (
            identity if mac or not oldmap.get("identity_key", "").startswith("mac:") else oldmap.get("identity_key")
        )
        conn.execute(
            """UPDATE inventory_assets SET identity_key=?, hostname=COALESCE(?,hostname), ip_address=COALESCE(?,ip_address),
        ...
        for key, newval in {
            ...
            if newval not in (None, "") and str(oldval or "") != str(newval):
                conn.execute(
                    "INSERT INTO inventory_history(asset_id,event_type,field_name,old_value,new_value,source,created_at) VALUES(?,?,?,?,?,?,?)",
        ...
        conn.execute(
            "INSERT INTO inventory_history(asset_id,event_type,field_name,new_value,source,created_at) VALUES(?,?,?,?,?,?)",
            (asset_id, "created", "asset", identity, source, now),
        )

    # Current hardware snapshot; empty values remain NULL.
    conn.execute(
        """INSERT INTO inventory_hardware(asset_id,cpu,ram_gb,gpu,motherboard,disk_json,serial_number,collected_at)
        ...
        (
            ...
            json.dumps(inventory.get("storage") or [], ensure_ascii=False),
    ...
    if isinstance(interfaces, list) and interfaces:
        ...
        for iface in interfaces[:100]:
            if isinstance(iface, dict):
                conn.execute(
                    "INSERT INTO inventory_interfaces(asset_id,interface_name,ip_address,mac_address,gateway,subnet,collected_at) VALUES(?,?,?,?,?,?,?)",
                    (
                        asset_id,
                        iface.get("name") or iface.get("interface_name"),
                        iface.get("ip") or iface.get("ip_address"),
                        _normalize_mac(iface.get("mac") or iface.get("mac_address")),
                        iface.get("gateway"),
                        iface.get("subnet") or iface.get("netmask"),
                        now,
                    ),
                )
    ...
    if programs:
        ...
        for program in programs[:1000]:
            if isinstance(program, dict) and program.get("name"):
                conn.execute(
                    "INSERT INTO inventory_software(asset_id,name,version,publisher,collected_at) VALUES(?,?,?,?,?)",
...
def _persist_device_inventory(dev: dict, inventory: dict, source: str | None = None):
    ...
    conn.execute(
        ...
        (
            dev["ip"],
            _normalize_mac(dev.get("mac")) or None,
            inventory["status"],
            source,
...
def _enrich_device_inventory(dev: dict, allow_deep: bool = False):
    ...
    if allow_deep and not dev.get("is_self") and dev.get("wmi_inventory", {}).get("status") != "Success":
        try:
            enriched = deep_discovery.integrate_discovery_flow(
                dev,
                credentials={
    ...
    dev["unified_inventory"] = {
        ...
        "inventory_source": (dev.get("wmi_inventory") or {}).get("inventory_source")
        if verified_wmi
        else (
            (dev.get("deep_inventory") or {}).get("inventory_source", "Verified deep protocol")
            if verified_deep
            else "Unverified network evidence"
        ),
...
def _analyst_correlation(dev):
    ...
    for src in sources:
        if src not in signals:
            signals.append(f"Discovery: {src}")
    if inv.get("verified"):
        signals.append(f"Doğrulanmış envanter: {inv.get('inventory_source', 'yetkili kaynak')}")
    if dev.get("vendor"):
        signals.append(f"Vendor: {dev.get('vendor')}")
    if dev.get("hostname"):
...
def _take_analyst_snapshot():
    ...
    offline = sum(d["status"] in {"offline", "stale"} for d in devices)
    ...
    conn = db_conn()
    conn.execute(
        "INSERT INTO analyst_snapshots(created_at,total,online,offline,unknown,health,completeness,security_review,payload) VALUES(?,?,?,?,?,?,?,?,?)",
        (
            time.time(),
            total,
            online,
...
def analyst_correlation(user: dict = Depends(get_current_user)):
    ...
    for d in _devices_cache.get("data") or []:
        a = _analyst_device(d)
        a["correlation"] = _analyst_correlation(d)
        a["review_priority"] = _review_priority(a)
        result.append(a)
...
def analyst_trends(limit: int = 30, user: dict = Depends(get_current_user)):
    ...
    conn = db_conn()
    rows = conn.execute(
        "SELECT created_at,total,online,offline,unknown,health,completeness,security_review FROM analyst_snapshots ORDER BY created_at DESC LIMIT ?",
        (limit,),
    ).fetchall()
    conn.close()
...
def analyst_snapshot(user: dict = Depends(get_current_user)):
    if user.get("role") != "admin":
        raise _AuthError(403, "Analiz snapshot için yönetici yetkisi gerekiyor.")
    _take_analyst_snapshot()
    _audit(user["username"], "analyst_snapshot", "Network intelligence snapshot")
    return {"ok": True}
...
def analyst_topology_evidence(user: dict = Depends(get_current_user)):
    ...
    for d in _devices_cache.get("data") or []:
        ip = d.get("ip")
        name = d.get("hostname") or d.get("friendly_name") or ip
                    ...
                    edges.append(
                        {
                            "source": name,
                            "target": str(peer),
                            "port": n.get("local_port") or n.get("port"),
        ...
        out.append(
            {
                ...
                "checks": [{"name": x[0], "ok": x[1]} for x in checks],
            }
        )
    return {"devices": out}
...
def analyst_report(user: dict = Depends(get_current_user)):
    ...
    lines = [
        ...
        f"Toplam cihaz: {len(devices)}",
        "",
    ]
    for d in devices:
        lines += [
            f"- {d['hostname'] or d['ip']} | {d['device_type']} | {d['status']} | Güven %{d['confidence']} | Envanter %{d['completeness']}"
        ]
...
ACADEMY_CONTENT = {
    "ip": {
        "title": "IP Adresi",
        "level": "Başlangıç",
        "summary": "Cihazın ağ üzerindeki mantıksal adresidir.",
        "lesson": [
            "Aynı ağdaki cihazların birbirini bulmasına yardımcı olur.",
            "IP değişebilir; bu yüzden envanter kimliği yalnız IP olmamalıdır.",
        ],
        "quiz": {
            "question": "Hangisi bir IPv4 adresidir?",
            "options": ["192.168.1.25", "AA:BB:CC:DD:EE:FF", "example.local", "255.255.255.255.255"],
            "answer": 0,
        },
    },
    "mac": {
        "title": "MAC Adresi",
        "level": "Başlangıç",
        "summary": "Ağ arayüzünün donanımsal adresidir.",
        "lesson": [
            "Yerel ağda arayüzleri ayırt etmek için kullanılır.",
            "Rastgeleleştirilmiş MAC adresleri nedeniyle tek başına kusursuz kimlik değildir.",
        ],
        "quiz": {
            "question": "MAC adresi en çok neyi temsil eder?",
            "options": ["Ağ arayüzünü", "DNS sunucusunu", "Web sitesini", "Kullanıcı şifresini"],
            "answer": 0,
        },
    },
    "dns": {
        "title": "DNS",
        "level": "Başlangıç",
        "summary": "Alan adlarını IP adreslerine çözümleyen sistemdir.",
        "lesson": [
            "Örneğin bir alan adının hangi IP'ye gittiğini bulmaya yardım eder.",
            "DNS arızasında internet bağlantısı varmış gibi görünüp siteler açılmayabilir.",
        ],
        "quiz": {
            "question": "DNS'in temel görevi nedir?",
            "options": ["Alan adını IP'ye çözümlemek", "RAM artırmak", "Firewall kurmak", "MAC değiştirmek"],
            "answer": 0,
        },
    },
    "dhcp": {
        "title": "DHCP",
        "level": "Başlangıç",
        "summary": "Ağ yapılandırmasını otomatik dağıtır.",
        "lesson": [
            "IP, ağ maskesi, gateway ve DNS gibi bilgileri dağıtabilir.",
            "Kiralama süresi dolduğunda cihaz yeni bir IP alabilir.",
        ],
        "quiz": {
            "question": "DHCP ne dağıtabilir?",
            "options": ["IP yapılandırması", "CPU çekirdeği", "Disk bölümü", "Kullanıcı parolası"],
            "answer": 0,
        },
    },
    "ports": {
        "title": "Portlar",
        "level": "Orta",
        "summary": "Ağ servislerinin mantıksal giriş noktalarıdır.",
        "lesson": [
            "TCP/UDP portları servisleri ayırt etmeye yardım eder.",
            "Açık port görmek tek başına güvenlik açığı bulunduğu anlamına gelmez.",
        ],
        "quiz": {
            "question": "443 numaralı port çoğunlukla hangi servisle ilişkilidir?",
            "options": ["HTTPS", "DHCP", "ARP", "DNS"],
            "answer": 0,
        },
    },
    "firewall": {
        "title": "Firewall",
        "level": "Orta",
        "summary": "Trafiği kurallara göre izin verir veya engeller.",
        "lesson": [
            "Kaynak, hedef, port ve protokol gibi ölçütlerle karar verebilir.",
            "Firewall durumu alınamıyorsa NetMon bunu tahmin etmemelidir.",
        ],
        "quiz": {
            "question": "Firewall'ın temel amacı nedir?",
            "options": [
                "Trafiği kurallarla kontrol etmek",
                "CPU hızını artırmak",
                "DNS kaydı oluşturmak",
                "IP üretmek",
            ],
            "answer": 0,
        },
    ...
    "ids": {
        "title": "IDS / IPS",
        "level": "Orta",
        "summary": "Şüpheli ağ davranışlarını algılama ve gerektiğinde önleme yaklaşımıdır.",
        "lesson": [
            "IDS olayları tespit edip uyarabilir.",
            "IPS tespit edilen trafiği politika kapsamında engelleyebilir.",
        ],
        "quiz": {
            "question": "IDS ile IPS arasındaki temel fark nedir?",
            "options": [
                "IPS önleme yeteneğine sahiptir",
                "IDS her zaman firewall'dır",
                "IPS DNS sunucusudur",
    ...
    "incident": {
        ...
        "level": "İleri",
        "summary": "Güvenlik olaylarını sistematik biçimde yönetme sürecidir.",
        "lesson": [
            "Tespit, analiz, sınırlama, düzeltme ve öğrenme adımlarını kapsar.",
            "Amaç yalnız olayı kapatmak değil, tekrarını azaltmaktır.",
        ],
        "quiz": {
            "question": "Olay müdahalesinde ilk önemli adımlardan biri nedir?",
...
def academy_modules(user: dict = Depends(get_current_user)):
    return {
        "modules": [
            {"id": k, "title": v["title"], "level": v["level"], "summary": v["summary"]}
            for k, v in ACADEMY_CONTENT.items()
        ]
    }
...
def academy_quiz(req: AcademyQuizRequest, user: dict = Depends(get_current_user)):
    ...
    return {
        "correct": correct,
        "answer": quiz["answer"],
        "message": "Doğru cevap." if correct else "Henüz değil. Açıklamayı tekrar inceleyin.",
    }
...
def _device_status(dev):
    return str(
        dev.get("status") or dev.get("connectivity_status") or ("online" if dev.get("online") else "unknown")
    ).lower()
...
def _analyst_recommendations(dev, exposure, completeness):
    ...
    if exposure["risk"] == "medium":
        rec.append(
            "Hassas görünen servisleri kurum politikasına göre doğrula; gereksiz servisleri kapatma kararı yönetici tarafından verilmelidir."
        )
    c = _classification(dev)
...
def _analyst_device(dev):
    ...
    return {
        ...
        "classification_source": dev.get("classification_source", "auto"),
        "evidence": evidence,
        "discovery_sources": dev.get("discovery_sources") or [],
...
def analyst_summary(user: dict = Depends(get_current_user)):
    ...
    health = 100
    if unknown:
        health -= min(20, len(unknown) * 2)
    if medium:
        health -= min(25, len(medium) * 3)
    if avg_loss and sum(avg_loss) / len(avg_loss) > 2:
        health -= 10
    ...
    return {
        ...
        "security": {
            "review_items": len(medium),
            "devices_with_exposure": len([d for d in analyzed if d["exposure"]["findings"]]),
        },
        "performance": {
            "average_latency_ms": round(sum(avg_latency) / len(avg_latency), 1) if avg_latency else None,
            "average_packet_loss": round(sum(avg_loss) / len(avg_loss), 2) if avg_loss else None,
        },
        "by_type": by_type,
...
def export_devices_csv(token: str | None = None, authorization: str | None = Header(None)):
    ...
    try:
            ...
            devices = [
                {
                    ...
                    "ip": r["last_ip"],
                    "vendor": r["last_vendor"],
                    "network": r["last_network"],
                    "status": r["connectivity_status"],
        ...
        for d in devices:
            ...
            disk_txt = (
                " · ".join(f"{ds.get('drive_letter', 'Disk')}: {ds.get('total_gb', 0)}GB" for ds in disks)
            ...
            if st == "online":
                st_text = "Çevrimiçi"
            elif st in ("offline", "stale"):
                st_text = "Çevrimdışı"
            elif st == "discovered":
                st_text = "Yanıt Doğrulanamadı"
            else:
                st_text = "Belirsiz"
...
def export_devices_save_to_disk(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        target_dir = (
            downloads_dir if os.path.isdir(downloads_dir) else (desktop_dir if os.path.isdir(desktop_dir) else home_dir)
        )
        filename = f"netmon_envanter_{datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S')}.csv"
...
def _inventory_failure_diagnostics(
    *,
    ...
    cause, actions = catalog.get(
        code,
        (
            existing.get("failure", {}).get("cause") or "Yönetim protokolü hedefte başarıyla tamamlanamadı.",
            existing.get("failure", {}).get("recommended_actions")
            or ["Ham hata ve bağlantı kanıtlarını hedef cihaz günlükleriyle eşleştirin."],
        ),
    )
    existing.update(
        {
...
def preflight_authorized_inventory(
    req: AuthorizedInventoryRequest, user: dict = Depends(require_permission("inventory.scan"))
):
    ...
    checks = [
        {
            "id": "target",
            "label": "Hedef doğrulama",
            "status": "pass",
            "detail": f"{ip} özel/yerel hedef olarak doğrulandı.",
        }
    ]
    ...
    checks.append(
        {
            "id": "ports",
            "label": "Yönetim kanalı",
            "status": "pass" if ports else "warn",
            "detail": ("Açık TCP portları: " + ", ".join(f"{p} ({observed[p]} ms)" for p in sorted(ports)))
            if ports
            else "TCP 22/135/445/5985/5986 portlarında yanıt alınmadı. SNMP UDP 161 ayrıca yetkilendirme adımında sınanır.",
    ...
    checks.append(
        {
            "id": "protocol",
            "label": "Protokol seçimi",
            "status": "pass" if protocol != "none" else "fail",
            "detail": f"Etkin protokol: {protocol.upper()}"
            if protocol != "none"
            else "Uygun yönetim protokolü belirlenemedi.",
        }
    )
    ...
    result = {
        ...
        "error_message": "Uygun yönetim kanalı bulunamadı.",
    }
    if protocol == "windows":
        ...
        credential_source = (
            ...
            if (WMI_USERNAME or WMI_PASSWORD)
            else "none"
        )
        ...
        checks.append(
            {
                "id": "credentials",
                "label": "Windows kimliği",
                "status": "pass" if credentials_ok else "fail",
                "detail": f"{account} hesabı ({'bu istek' if credential_source == 'request' else 'DPAPI kaydı'}) kullanılacak."
                if credentials_ok
                else "Uzak Windows testi için kullanıcı adı ve parola bulunamadı.",
            }
        )
        ...
        if cooldown:
            ...
            scanner = WmiNetworkScanner(
                username=account, password=password, timeout=req.timeout, verify_tls=WINRM_VERIFY_TLS
            )
            ...
            result = {
                ...
                "error_message": "Windows kullanıcı adı ve parola gerekli.",
            }
    elif protocol == "ssh":
        ...
        checks.append(
            {
                ...
                "label": "SSH kimliği",
                "status": "pass" if credentials_ok else "fail",
                "detail": f"{account} hesabı kullanılacak."
                if credentials_ok
                else "SSH kullanıcı adı ve parola bulunamadı.",
            }
        )
        result = (
            deep_discovery.test_ssh_access(ip, account, password, timeout=max(5, min(req.timeout, 60)))
            if credentials_ok
            else {
                "status": "Unavailable",
                "error_code": "missing_credentials",
                "error": "SSH kullanıcı adı ve parola gerekli.",
            }
        )
    elif protocol == "snmp":
        ...
        checks.append(
            {
                "id": "credentials",
                "label": "SNMP kimliği",
                "status": "pass" if community else "fail",
                "detail": "Salt-okuma community hazır; değer güvenlik nedeniyle gösterilmiyor."
                if community
                else "SNMP community bulunamadı.",
            }
        )
        result = (
            deep_discovery.scan_snmp_deep(ip, community=community, timeout=max(1, min(req.timeout, 10)))
            if community
            else {"status": "Unavailable", "error_code": "missing_credentials", "error": "SNMP community gerekli."}
        )

    authorized = result.get("status") == "Success"
    diagnostics = (
        _inventory_failure_diagnostics(
            ip=ip,
            requested_protocol=requested_protocol,
            protocol=protocol,
            result=result,
    ...
    checks.append(
        {
            "id": "authorization",
            "label": "Gerçek yetkilendirme",
            "status": "pass" if authorized else "fail",
            "detail": (
                f"{protocol.upper()} bağlantısı ve okuma yetkisi doğrulandı."
                if authorized
                else diagnostics.get("cause")
                or result.get("error_message")
                or result.get("error")
                or "Yetkilendirme başarısız."
            ),
    ...
    _audit(
        user["username"],
        "inventory_preflight",
        f"target={ip} protocol={protocol} status={'ready' if authorized else 'failed'}",
        success=authorized,
    )
    return {
        "ok": authorized,
        ...
        "summary": "Hedef yetkili envanter taramasına hazır."
        if authorized
        else "Hazırlık testi bir engel tespit etti.",
    }


def scan_authorized_device_inventory(
    req: AuthorizedInventoryRequest, user: dict = Depends(require_permission("inventory.scan"))
):
    if not 5 <= req.timeout <= 60:
        return JSONResponse(status_code=400, content={"error": "Zaman aşımı 5-60 saniye arasında olmalıdır."})
    ...
    if protocol == "auto":
        if ports.intersection({135, 445, 3389, 5985, 5986}) or device.get("type") in {
            "computer",
            "pc",
            "laptop",
            "server",
            ...
            protocol = "ssh"
        elif 161 in ports or device.get("type") in {
            "router",
            "switch",
            "access_point",
            "firewall",
...
def device_scan_loop(stop_event: threading.Event):
    while not stop_event.is_set():
        ...
        try:
            ...
            _discovery_schedule_state.update(
                last_status="failed", last_error="Beklenmeyen keşif hatası; uygulama günlüğünü inceleyin."
            )
        finally:
...
def get_admin_xoc_metrics(user: dict = Depends(require_permission("security.manage"))):
    ...
    if HAS_PSUTIL and psutil:
        try:
            ...
            active_speeds = [
                stat.speed for stat in psutil.net_if_stats().values() if stat.isup and stat.speed and stat.speed > 0
            ]
    ...
    infrastructure_online = sum(
        1
        for device in (_devices_cache.get("data") or [])
...
def add_to_blacklist(req: BlacklistRequest, user: dict = Depends(require_permission("security.manage"))):
    ...
    return {
        "ok": True,
        "message": f"{ip} izleme listesine eklendi; firewall engeli uygulanmadı.",
        "blacklisted_ips": list(_blacklist_ips),
    }
...
def start_dos_simulation(req: DosSimulateRequest, user: dict = Depends(require_permission("security.manage"))):
    ...
    if not _is_allowed_inventory_ip(target_ip) or req.intensity not in {"low", "medium", "high"}:
        return JSONResponse(
            status_code=400, content={"error": "Yalnızca yerel/özel IPv4 hedefleri ve geçerli yoğunluk kullanılabilir."}
        )
    ...
    try:
        ...
        if _devices_cache.get("data"):
            await websocket.send_text(
                json.dumps(
                    {"type": "devices", "devices": _devices_cache["data"], "ts": _devices_cache.get("ts", time.time())}
                )
            )
        while True:
...
def get_all_settings():
    ...
    for bool_key in ("public_ip_lookup", "winrm_verify_tls", "ncm_auto_backup_enabled", "smtp_tls"):
        ...
        result[bool_key] = (
            raw_bool if isinstance(raw_bool, bool) else str(raw_bool).lower() in ("1", "true", "yes", "on")
        )
    return result
...
def _runtime_network_visibility() -> dict:
    ...
    return {
        ...
        "note": (
            ...
            if elevated
            else "NetMon yükseltilmiş işletim sistemi belirteciyle çalışmıyor; bazı sistem işlemlerinin adı veya PID bilgisi görünmeyebilir."
        ),
    }
...
def get_top_talkers(user: dict = Depends(get_current_user)):
    ...
    conn = db_conn()
    row = conn.execute(
        "SELECT ts, wifi_sent, wifi_recv, eth_sent, eth_recv FROM traffic ORDER BY ts DESC LIMIT 1"
    ).fetchone()
    conn.close()
...
def ncm_backup_loop(stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        if not (SSH_USERNAME and SSH_PASSWORD):
            _ncm_auto_state.update(
                last_status="missing_credentials", errors=["Ayarlar'da SSH salt-okuma hesabı eksik."]
            )
            stop_event.wait(min(300, max(30, NCM_BACKUP_INTERVAL)))
            continue
        candidates = [
            d
            for d in _devices_cache.get("data", [])
            if d.get("ip")
            and (d.get("is_gateway") or d.get("type") in {"switch", "router", "firewall", "access_point"})
        ]
        checked = changed = 0
        errors = []
        for dev in candidates:
            ...
            try:
                ...
                previous = conn.execute(
                    "SELECT config_hash FROM device_configs WHERE ip=? ORDER BY created_at DESC LIMIT 1", (ip,)
                ).fetchone()
                checked += 1
                if not previous or previous[0] != cfg_hash:
                    ...
                    conn.execute(
                        ...
                        (
                            ip,
                            dev.get("hostname") or dev.get("friendly_name") or ip,
                            dev.get("type") or "unknown",
                            config_text,
                            cfg_hash,
                            f"Otomatik-{datetime.fromtimestamp(now).strftime('%Y%m%d-%H%M%S')}",
                            now,
                        ),
                    )
                    if previous:
                        conn.execute(
                            ...
                            (now, "warning", f"Konfigürasyon değişikliği inceleme bekliyor: {ip}", "NCM"),
                        )
                    conn.commit()
                    changed += 1
                conn.close()
        ...
        _ncm_auto_state.update(
            last_run=time.time(),
            last_status="completed" if not errors else "completed_with_errors",
            checked=checked,
            changed=changed,
            errors=errors[:20],
        )
...
def get_operations_report(user: dict = Depends(require_permission("reports.view"))):
    ...
    snapshots = conn.execute(
        "SELECT total, online, health FROM analyst_snapshots WHERE created_at>=? ORDER BY created_at", (since,)
    ).fetchall()
    alert_rows = conn.execute(
        "SELECT level, message, COUNT(*) c FROM alerts WHERE ts>=? GROUP BY level, message ORDER BY c DESC LIMIT 8",
        (since,),
    ).fetchall()
    traffic_row = conn.execute(
        "SELECT AVG(wifi_sent+eth_sent), AVG(wifi_recv+eth_recv), MAX(wifi_sent+eth_sent), MAX(wifi_recv+eth_recv) FROM traffic WHERE ts>=?",
        (since,),
    ).fetchone()
...
def get_locations_summary(user: dict = Depends(require_permission("locations.view"))):
    ...
    return {
        ...
        "assets": assets,
        "can_manage": _has_permission(user, "locations.manage"),
        "naming_example": "İstanbul Merkez > A Blok > Kat 3 > Kabinet 3A",
    }
